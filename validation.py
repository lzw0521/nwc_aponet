...
from common import append_loss_to_file,euclidean_distance


def validate(instances, p_net, no_agent, device):

    total_distances = [0 for _ in range(instances.shape[0])]
    ...
    reward,total_distances,_ = get_reward(action, instances, no_agent)  # reward: tensor [batch, 1]
    # print('Validation result:', format(sum(reward)/batch_size, '.4f'))
    print(f"reward:{reward}")
    return sum(reward)/batch_size,reward[0],total_distances[0]
