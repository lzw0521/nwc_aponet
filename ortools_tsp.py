...
def solve(instance):
    ...
    #     print_solution(manager, routing, solution)
     ...
     # Print solution on console.
    if solution:
        # Initialize an empty list to collect the routes.
        routes = []
        for vehicle_id in range(data['num_vehicles']):
            ...
            while not routing.IsEnd(index):
                ...
                index = solution.Value(routing.NextVar(index))
            node_index = manager.IndexToNode(index)  # 获取终点的坐标
            route.append(data['locations'][node_index])  # 添加终点的 (x, y) 坐标
            routes.append(route)
            # print('Route for vehicle {}:\n{}'.format(vehicle_id, route))
        
        # Optionally, return the routes
